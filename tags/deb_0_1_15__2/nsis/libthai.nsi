...
!define ENV_SETFILE  "libthaienv.sh"

;set BASEPATH variable name
!define BASEPATH "LIBTHAI_BASEPATH"
;--------------------------------
...
;The stuff to install
 ...
 Section /o "${PROGNAME}-data" datasec
  ;SectionIn RO
  ;Set output path to the installation directory.
  SetOutPath "$INSTDIR\share\${PROGNAME}"
  ;Put file there
  File "${BINSRCDIR}\share\${PROGNAME}\*.*"
  ;Write component info.
  WriteRegStr HKLM "SOFTWARE\${PROGNAME}\components" "data" "1"
 SectionEnd
 ...
 Section /o "${PROGNAME}-doc" docsec
  ...
  ;Put file there
  File /r "${BINSRCDIR}\share\doc\${PROGNAME}"
   ;Write component info.
...
Push $R1
  ...
  # Check if runtimesec was just selected then select datasec and develsec.
  SectionGetFlags ${runtimesec} $R0
  IntOp $R0 $R0 & ${SF_SELECTED}
  ...
  # Select runtimesec if datasec or develsec was selected.
  ...
  # Were datasec or develsec just unselected?
  CheckDependentSections:
 
  SectionGetFlags ${datasec} $R0
  IntOp $R0 $R0 & ${SF_SELECTED}
  ...
  # Unselect datasec and develsec if runtimesec was unselected.
